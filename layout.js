...
const MEGA_MENU_VISIBLE_SUBS = 6;

function renderCategoryMenu() {
  return Object.entries(CATEGORY_STRUCTURE).map(([cat, subs]) => {
    const visible = subs.slice(0, MEGA_MENU_VISIBLE_SUBS);
    const remaining = subs.length - visible.length;
    return `
      <div class="mega-menu-col">
          <a href="shop.html?category=${encodeURIComponent(cat)}" class="block text-sm font-bold text-white hover:text-red-400 transition mb-2">
              <i class="${CATEGORY_ICONS[cat] || 'fa-solid fa-gear'} text-red-500 mr-1.5"></i>${cat}
          </a>
          <ul class="space-y-1">
              ${visible.map(sub => `<li><a href="shop.html?category=${encodeURIComponent(cat)}&sub=${encodeURIComponent(sub)}" class="text-xs text-slate-400 hover:text-red-400 transition">${sub}</a></li>`).join('')}
              ${remaining > 0 ? `<li><a href="shop.html?category=${encodeURIComponent(cat)}" class="text-xs text-red-500 font-semibold hover:text-red-400 transition">+${remaining} more &rarr;</a></li>` : ''}
              ${subs.length === 0 ? `<li><a href="shop.html?category=${encodeURIComponent(cat)}" class="text-xs text-slate-500 italic">View all &rarr;</a></li>` : ''}
          </ul>
      </div>`;
  }).join('');
}
...
function renderMobileCategoryMenu() {
  return Object.entries(CATEGORY_STRUCTURE).map(([cat, subs]) => `
      <details class="border-b border-slate-800 pb-2">
          <summary class="text-sm font-semibold text-white py-1 cursor-pointer">${cat}</summary>
          <div class="pl-4 pt-1 flex flex-col space-y-1">
              ${subs.map(sub => `<a href="shop.html?category=${encodeURIComponent(cat)}&sub=${encodeURIComponent(sub)}" class="text-xs text-slate-400 hover:text-red-400">${sub}</a>`).join('')}
          </div>
      </details>`).join('');
}
  ...
  <header class="sticky top-0 z-40 bg-slate-950 border-b border-slate-800 shadow-md">
      <div class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 h-20 flex items-center justify-between gap-4">
          ...
          <nav class="hidden md:flex items-center space-x-6">
              <div class="relative" id="categoriesMenuWrap">
                  <button id="categoriesMenuBtn" class="text-sm font-semibold text-slate-300 hover:text-white transition flex items-center space-x-1.5">
                      <i class="fa-solid fa-bars-staggered text-red-500"></i>
                      <span>All Categories</span>
                      <i class="fa-solid fa-chevron-down text-[10px] mt-0.5"></i>
                  </button>
                  <div id="categoriesMegaMenu" class="hidden absolute top-full left-0 mt-3 w-[760px] max-w-[85vw] max-h-[75vh] overflow-y-auto bg-slate-950 border border-slate-800 rounded-xl shadow-2xl p-6 grid grid-cols-3 gap-6">
                      ${renderCategoryMenu()}
                  </div>
              </div>
              ${links}
      ...
      <nav id="mobileMenu" class="md:hidden hidden border-t border-slate-800 bg-slate-950 px-4 py-3 flex flex-col space-y-3">
          ...
          <div class="border-t border-slate-800 pt-3 mt-1">
              <p class="text-xs uppercase tracking-widest text-red-500 font-bold mb-2">All Categories</p>
              <div class="flex flex-col space-y-2">${renderMobileCategoryMenu()}</div>
          </div>
      </nav>
  </header>`;
}
...
function initLayout(activePage) {
  ...
  const categoriesMenuWrap = document.getElementById('categoriesMenuWrap');
  categoriesMenuBtn.addEventListener('click', (e) => {
    e.stopPropagation();
    categoriesMegaMenu.classList.toggle('hidden');
  });
  document.addEventListener('click', (e) => {
    if (!categoriesMenuWrap.contains(e.target)) categoriesMegaMenu.classList.add('hidden');
  });
